...
GearBlocks BepInEx work is tracked as source-template infrastructure for direct Overlay Forge-owned plugin features and optional GearLib-based plugin research. Future work should keep repo-owned templates under `gearblocks-bepinex-templates/`, keep generated local projects under the ignored `gearblocks-bepinex-workspace/`, and avoid committing GearLib, BepInEx, GearBlocks interop, Unity, or compiled plugin DLLs. GearLib must remain documented as a third-party user-installed requirement only for GearLib-based plugin paths. The direct `OverlayForgeGearBlocksPlugin` path does not depend on GearLib.
